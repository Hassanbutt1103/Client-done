...
import { FaBars, FaMoneyBillWave, FaChartLine, FaCalculator, FaFileInvoiceDollar, FaSignOutAlt } from 'react-icons/fa';
import { useNavigate, NavLink } from 'react-router-dom';
import novaLogo from '../../assets/Nova Logo VP.png';
import API_ENDPOINTS from '../../config/api';

const financialNavLinks = [
  { name: 'Financial Overview', icon: <FaMoneyBillWave className="text-white" />, path: '/financial/overview' },
  { name: 'Revenue Analysis', icon: <FaChartLine className="text-white" />, path: '/financial/revenue' },
  { name: 'Budget Planning', icon: <FaCalculator className="text-white" />, path: '/financial/budget' },
  { name: 'Invoice Management', icon: <FaFileInvoiceDollar className="text-white" />, path: '/financial/invoices' },
];

const Sidebar = ({ collapsed, setCollapsed, mobileSidebarOpen, setMobileSidebarOpen }) => {
  const navigate = useNavigate();
  const user = JSON.parse(localStorage.getItem('user') || '{}');
  const userName = user.name || 'Financial User';
  const userRole = user.role ? user.role.charAt(0).toUpperCase() + user.role.slice(1) : 'Financial';
