...
import { FaBars, FaCogs, FaChartLine, FaTools, FaProjectDiagram, FaSignOutAlt } from 'react-icons/fa';
import { useNavigate, NavLink } from 'react-router-dom';
import novaLogo from '../../assets/Nova Logo VP.png';
import { API_ENDPOINTS } from '../../config/api';

const engineeringNavLinks = [
  { name: 'Engineering Overview', icon: <FaCogs className="text-white" />, path: '/engineering/overview' },
  { name: 'Project Management', icon: <FaProjectDiagram className="text-white" />, path: '/engineering/projects' },
  { name: 'Performance Metrics', icon: <FaChartLine className="text-white" />, path: '/engineering/metrics' },
  { name: 'Equipment Status', icon: <FaTools className="text-white" />, path: '/engineering/equipment' },
];
...
const Sidebar = ({ collapsed, setCollapsed, mobileSidebarOpen, setMobileSidebarOpen }) => {
  ...
  const userRole = user.role ? user.role.charAt(0).toUpperCase() + user.role.slice(1) : 'Engineering';
