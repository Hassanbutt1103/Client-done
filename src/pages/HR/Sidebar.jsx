...
import { FaBars, FaUsers, FaChartBar, FaCalendarAlt, FaUserTie, FaSignOutAlt } from 'react-icons/fa';
import { useNavigate, NavLink } from 'react-router-dom';
import novaLogo from '../../assets/Nova Logo VP.png';
import { API_ENDPOINTS } from '../../config/api';

const hrNavLinks = [
  { name: 'HR Overview', icon: <FaUsers className="text-white" />, path: '/hr/overview' },
  { name: 'Employee Management', icon: <FaUserTie className="text-white" />, path: '/hr/employees' },
  { name: 'Performance Metrics', icon: <FaChartBar className="text-white" />, path: '/hr/metrics' },
  { name: 'Schedule Management', icon: <FaCalendarAlt className="text-white" />, path: '/hr/schedule' },
];
...
const Sidebar = ({ collapsed, setCollapsed, mobileSidebarOpen, setMobileSidebarOpen }) => {
  ...
  const userRole = user.role ? user.role.charAt(0).toUpperCase() + user.role.slice(1) : 'HR';
