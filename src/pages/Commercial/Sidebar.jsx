...
import { FaBars, FaBuilding, FaChartBar, FaHandshake, FaUserTie, FaSignOutAlt } from 'react-icons/fa';
import { useNavigate, NavLink } from 'react-router-dom';
import novaLogo from '../../assets/Nova Logo VP.png';
import { API_ENDPOINTS } from '../../config/api';

const commercialNavLinks = [
  { name: 'Commercial Overview', icon: <FaBuilding className="text-white" />, path: '/commercial/overview' },
  { name: 'Sales Analytics', icon: <FaChartBar className="text-white" />, path: '/commercial/sales' },
  { name: 'Client Management', icon: <FaUserTie className="text-white" />, path: '/commercial/clients' },
  { name: 'Partnership Deals', icon: <FaHandshake className="text-white" />, path: '/commercial/deals' },
];
...
const Sidebar = ({ collapsed, setCollapsed, mobileSidebarOpen, setMobileSidebarOpen }) => {
  ...
  const userRole = user.role ? user.role.charAt(0).toUpperCase() + user.role.slice(1) : 'Commercial';
